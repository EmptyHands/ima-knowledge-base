...
| 版本 | 日期 | 变更 |
|------|------|------|
| v1.5.0 | 2026-08-22 | 新增 DEV-022:详细日志 — 可开关的单文件 detail.log(内部思考/工具调用/链路/state/RAG 查询与 chunk 结果),关闭时零开销;配置经 .env.example 与 .gitignore 纳入版本管理 |
| v1.4.0 | 2026-08-22 | 新增 DEV-019:nginx 网关 + Docker 多后端实例横向扩展(least_conn + SSE 透传,被动健康检查);langgraph 人机交互中断态从内存换 AsyncRedisSaver 暂存 Redis,resume 落到任意实例可恢复,Redis 不可用自动降级内存;SQLite 开 WAL + busy timeout 支持共享卷多实例并发(纯后端 + 部署层,前端零改动) |
| v1.3.0 | 2026-08-21 | 新增 DEV-012:问答流水线 langgraph 化 — 检索/判定/人机交互/回答四节点显式状态图, RAG 无可靠结果经 interrupt 挂起询问用户, 确认后循环回检索节点强制联网重检, 终止分支防死循环(纯后端, SSE 契约与前端不变) |
| v1.2.0 | 2026-08-17 | 新增 DEV-011:统一消息抽象(Pydantic ChatMessage 贯穿路由/answer_agent/LLM 适配器,LLM 请求升级为结构化多轮 messages)+ DEV-015:对话摘要压缩(窗口外历史增量压缩为摘要,检索时并行生成并以 system 消息注入);修复 DEV-018:检索片段与网络结果未进入 LLM 请求(DEV-011 升级回归)(纯后端, 前端不变) |
...
| v1.0.2 | 2026-08-13 | 修复批次:引用展示从"有依据/无来源"二元对立改为双级展示(DEV-007);会话管理框新增/删除按钮有几率消失(DEV-008);LLM 流式输出为空时落库空消息导致空白气泡(DEV-017);会话标题在兜底反问分支显示 agent 回复原文(DEV-009) |
| v1.0.1 | 2026-08-11 | 修复批次:README 流程图无法渲染(DEV-001);检索无结果兜底 — 双向量 BM25 降级 + 空库/无结果反问 + 确认后强制联网(DEV-004);兜底回复无 chunk 事件,前端不刷新不显示答案(DEV-005);引用校验 Jaccard 被长 chunk 稀释误标无来源,改包含率指标(DEV-002);双向量迁移后存量文档向量丢失,启动对账自愈(DEV-006);短行碎片 chunk 打包为内容页 + 引用上下文回溯与死循环修复(DEV-003/DEV-002 残留) |
...
- 多实例模式:MCP SSE 会话注册表 per-instance,`/mcp/sse` 长连接与 `/mcp/messages` 被分发到不同实例时查不到会话(可后续 nginx `ip_hash` 对 /mcp/* 做粘性路由)
- 多实例模式:SQLite 共享卷仅支持单机多实例,跨机横向扩展需 PostgreSQL(部署上云方向)
- 多实例模式:Redis 单点,生产需 Sentinel/集群
